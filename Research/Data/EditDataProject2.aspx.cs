...
public partial class Data_EditDataProject2 : System.Web.UI.Page
{
	...
	{
		Response.Redirect("~/Data/DataProject2.aspx");
	}
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		if (!IsPostBack)
		{
			...
			if (Request.QueryString["pk"] != null)
			{
				...
				//Check to see if IntHx is part of this study
				SQL_utils sql = new SQL_utils("backend");
				int hasIntHx = sql.IntScalar_from_SQLstring("select count(*) from tblStudymeas where measureID = 749 and studyID=" + Master.Master_studyID.ToString());
				panelIntHx.Visible = (hasIntHx>=1) ? true : false;
				sql.Close();


				//TODO Check to see if project is approved


				//TODO Check to see if the study has reliability measures.
				//If so, display the "include reliability" checkbox and "refresh" button
	...
	protected void LoadProjects(int dataproj_pk)
	{
		...
		if (dataproj_pk > 0)
		{
			...
			Summary_HtmlEditor.Html = dt.Rows[0].Field<string>("projSummary");
			//editor.Content = dt.Rows[0].Field<string>("projSummary");

			lblTitle.Text = dt.Rows[0].Field<string>("projTitle");
			txtTitle.Text = dt.Rows[0].Field<string>("projTitle");
		...
		//Check to see if IntHx is in this project

		int studyHasIntHx = sql.IntScalar_from_SQLstring("select count(*) from uwautism_research_backend..tblstudymeas where measureID = 749 and studyID =" + Master.Master_studyID.ToString());
		int projHasIntHx = sql.IntScalar_from_SQLstring("select count(*) from dp.IntHxPARAMS where dataproj_pk = " + dataproj_pk.ToString());

		if(studyHasIntHx > 0){
			panelIntHx.Visible = true;
		}
		else {
			panelIntHx.Visible = false;
		}


		if (projHasIntHx > 0 )
		{
			btnIntHxADD.Visible = false;
			btnIntHxEDIT.Visible = true;
		} else 		{
			gridViewIntHx.Visible = false;
			btnIntHxADD.Visible = true;
			btnIntHxEDIT.Visible = false;
		}
	...
	protected void LoadSupervisorsDDL(int studyID, int supervisorID)
{
	...
	protected void Load_for_View_Subjects(int dataproj_pk, SQL_utils sql)
	{
		...
		DataTable dt2 = sql.DataTable_from_SQLstring("exec dp.spDataProject_Subjects  " + dataproj_pk.ToString() + ", 0" );
		pivotSubjects1.DataSource = dt2;

		pivotSubjects2.DataSource = dt2;
	...
	protected void Load_for_View_Measures(int dataproj_pk, SQL_utils sql)
	{
		...
		string sqlcode = "select a.measureID, Meascat as Category , Measname as Measure " +
			" , a.studymeasID, c.timepointID, timepoint_text " +
			" , a.studymeasname " +
			" from[dp].[vwMeasures_by_FullStudy] a " +
			"    join (select* from dp.Meas where dataproj_pk = " + dataproj_pk.ToString() + " ) b ON a.studymeasID = b.studymeasID and a.measureID = b.measureID " +
			" join uwautism_research_backend..tblstudymeas c ON a.studymeasID = c.studymeasID " +
			" join uwautism_research_backend..tbltimepoint d ON c.timepointID = d.timepointID" ;


		DataTable dt3 = sql.DataTable_from_SQLstring(sqlcode);
		pivotMeasures1.DataSource = dt3;
...
//	}
	...
	//Response.Redirect("~/Data/EditDataProject2.aspx?pk=" + dataproj_pk);
	//}
	...
	//	//HtmlEditor.Html = litSummary.Text;
	...
	protected void 	btn_Edit_Command(object sender, CommandEventArgs e)
	{
		...
		if (e.CommandName == "Edit"){
			...
			if (e.CommandArgument.ToString() == "Title")
			{

			}
			else if (e.CommandArgument.ToString() == "Subjects")
			{
				LoadSubjects_for_Edit(Master.Master_studyID);

				panelSubjects_VIEW.Visible = false;
				panelSubjects_EDIT.Visible = true;
			}
			else if (e.CommandArgument.ToString() == "Measures")
			{
				panelMeasures_VIEW.Visible = false;
				panelMeasures_EDIT.Visible = true;

			}
			else if (e.CommandArgument.ToString() == "IntHx")
			{
				panel_IntHx_VIEW.Visible = false;
				panel_IntHx_EDIT.Visible = true;
			}
		...
		else if (e.CommandName == "Save")
		{
			...
			else if (e.CommandArgument.ToString() == "Summary")
			{
				SaveProject("Summary");
			}
			else if (e.CommandArgument.ToString() == "Subjects")
			{
				SaveProject("Subjects");
	...
	protected void SaveProject(string section)
	{
		SQL_utils sql = new SQL_utils();
		...
		if(section=="Title")
		{
			...
			sql.NonQuery_from_ProcName("dp.spDataProject_UPDATE_title", p);
			lblTitle.Text = txtTitle.Text;
		}
		else if (section == "Summary")
		{
			p.Add(sql.CreateParam("projSummary", Summary_HtmlEditor.Html, "text"));
			sql.NonQuery_from_ProcName("dp.spDataProject_UPDATE_summary", p);
		}
		else if (section == "Subjects")
		{
			DataTable dt_sel_subj = DataTable_GetSelectedSubjects(gvSubjects, "");
			p.Add(sql.CreateParam("sel_subj", dt_sel_subj));
			sql.NonQuery_from_ProcName("dp.spDataProject_UPDATE_subjects", p);
		}
		...
		else if (section == "Measures")
		{
			...
			sql.NonQuery_from_ProcName("dp.spDataProject_UPDATE_measures", p);
		}
		...
		Response.Redirect("~/Data/EditDataProject2.aspx?pk=" + Request.QueryString["pk"]);
