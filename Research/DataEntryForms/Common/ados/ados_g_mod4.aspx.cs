...
{
	/// <summary>
	/// Summary description for ados_g_mod4.
	/// </summary>
	public partial class ados_g_mod4 : System.Web.UI.Page
	{
		...
		{    
			...
			this.sqlInsert.CommandText = "INSERT INTO gen_ados (indid, ADOSdate, ADOSclin, ADOSmod, ADOSver, verified) VALU" +
				"ES (@indid, @ADOSdate, @ADOSclin, 4, \'G\', @verified); SELECT * FROM gen_ados WHE" +
				"RE ados_pk = @@IDENTITY;";
			...
			// sqlLookup
			// 
			this.sqlLookup.CommandText = "SELECT GEN_ADOS.* FROM GEN_ADOS WHERE (ADOSmod = 4) AND (indid = @indid) AND (ADO" +
				"Sver = \'G\')";
