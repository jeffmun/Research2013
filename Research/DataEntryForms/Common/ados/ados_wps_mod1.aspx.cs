...
{
	/// <summary>
	/// Summary description for ados_wps_mod1.
	/// </summary>
	public partial class ados_wps_mod1 : System.Web.UI.Page
	{
		...
		{    
			...
			this.sqlInsert.CommandText = "INSERT INTO gen_ados (indid, ADOSdate, ADOSclin, ADOSmod, ADOSver, verified) VALU" +
				"ES (@indid, @ADOSdate, @ADOSclin, 1, \'WPS\', @verified); SELECT * FROM gen_ados W" +
			...
			// sqlLookup
			// 
			this.sqlLookup.CommandText = "SELECT GEN_ADOS.* FROM GEN_ADOS WHERE (ADOSmod = 1) AND (indid = @indid) AND (ADO" +
				"Sver = \'WPS\')";
