...
{
	/// <summary>
	/// Summary description for edst3_adi.
	/// </summary>
	public partial class edst3_adi : System.Web.UI.Page
	{
		...
		{
            //add  onload="edst3_adi_onLoad()" to the master page's <body> tag
            HtmlControl body = (HtmlControl)Master.FindControl("oBody");
            body.Attributes["onload"] = "edst3_adi_onLoad()";
		}
		...
		{    
			...
			this.PreRender += new System.EventHandler(this.edst3_adi_PreRender);
		...
		protected void DataEntryController1_DoScore(object sender, System.EventArgs e)
		{
			try 
			{
				...
				this.sqlScoreADI.Parameters["@adi_pk"].Value = DataEntryController1.PrimaryKeyVal;
				this.sqlScoreADI.Parameters["@studyid"].Value = 3;

				this.sqlDataConnection.Open();

				// this statement can throw an SqlException if any errors occur in spScoreMullen
				this.sqlScoreADI.ExecuteNonQuery();
		...
		protected void edst3_adi_PreRender(object sender, System.EventArgs e)
		{
