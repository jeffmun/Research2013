...
namespace Measures
{
	public partial class EditConsentForm : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				...
				if (oReader.Read())
				{
					...
					{
						apppath = Request.Url.Authority ;
					}

					tdInfo.InnerHtml += "<div style=\"padding: 4px;\"><a href=\"" + apppath + "/Measures/EditSubject.aspx?SubjID=" + nSubjID.ToString() + "\" class=\"smalltext\">Edit Subject - " + sSubjID.ToString() + "</a></div>";
					tdInfo.InnerHtml += "<div style=\"padding: 4px;\"><a href=\"" + apppath + "/Measures/ActionsBySubject.aspx?SubjectID=" + nSubjID.ToString() + "\" class=\"smalltext\">Action Summary - " + sSubjID.ToString() + "</a></div>";
					tdInfo.InnerHtml += "<div style=\"padding: 4px;\"><a href=\"" + apppath + "/Measures/MeasuresBySubject.aspx?SubjectID=" + nSubjID.ToString() + "\" class=\"smalltext\">Measure Summary - " + sSubjID.ToString() + "</a></div>";
					tdInfo.InnerHtml += "<div style=\"padding: 4px;\"><a href=\"" + apppath + "/Household/ViewHousehold.aspx?HouseholdID=" + nHouseholdID.ToString() + "\" class=\"smalltext\">View Household - " + sSubjID.ToString() + "</a></div>";
					tdInfo.InnerHtml += "<div style=\"padding: 4px;\"><a href=\"" + apppath + "/Household/EditHousehold.aspx?HouseholdID=" + nHouseholdID.ToString() + "\" class=\"smalltext\">Edit Household - " + sSubjID.ToString() + "</a></div>";
		...
		{
			//fill a dataset with data tables.  
			//The first one holds assigned consent items.  
			//The second holds allowed consent item options.

			try
			{
				string sql = "select * from vwSubjConsentItems where SubjConsentID = @SubjConsentID;" +
							 "select *  from vwConsentFormVersion_ConsentItemOption_with_0 where ConsentFormVersion_ConsentItemID in " + 
							 "	(select ConsentFormVersion_ConsentItemID  from vwSubjConsentItems where SubjConsentID = @SubjConsentID) ;";
		...
		protected void SaveChanges(object sender, System.EventArgs e)
		{
			...
			foreach (RepeaterItem ri in rpConsentItems.Items)
			{
				...
				oCmd.CommandText = "update tblSubjConsentItem " +
					"set ConsentItemOptionID = @ConsentItemOptionID, cioID = @cioID, ItemDateSigned = @ItemDateSigned, " +
				...
				oCmd.Parameters.AddWithValue("@ConsentItemOptionID", Convert.ToInt32(ddl.SelectedValue));
				oCmd.Parameters.AddWithValue("@cioID", Convert.ToInt32(ddl.SelectedValue));
				...
				//string newsql = "update tblSubjConsentItem set ConsentItemOptionID = " + ddl.SelectedValue.ToString() + " , ItemDateSigned = " + d.Text  +
				//         ", ItemTimeSigned = " + t.Text + ", SubjConsentItemNotes = " + n.Text + "where SubjConsentItemID = " + ht.Value.ToString() + "<br/>";
