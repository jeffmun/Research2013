...
namespace uwac
{
	...
	public class DataImporter
	{
		...
		protected void CreateTableForImport(string ID, int studymeasID, string filepath, string filename, string tblname)
		{
			...
			else
			{
				...
				if (settings.importfiletype == ImportFiletype.csv | settings.importfiletype == ImportFiletype.tsv)
				{
					...
					//if (settings.tblname == "ALL_SleepEnv_Sound")
					//{
					//	DataTable dt_agg = Actigraph.AggregrateSoundData(_imported_dt, settings.tblname);
					//	result = sql.BulkInsert(dt_agg, settings.tblname);
					//}
					//else
					//{
					//	result = sql.BulkInsert(_imported_dt, settings.tblname);
					//}
		...
		#region Create the DataTable from the imported file
		...
		////Needed for SPSSsav 
		//public static DataTable AddNeededColumnsBeforeImport(DataTable dt, DataImportSettings settings)
		//{
		//	bool hasindexnum = dt.ContainsColumnName("indexnum");
