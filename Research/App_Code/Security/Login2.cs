...
namespace AutismCenterBase.Security
{
	public class Login2
	{
		...
		public Login2(string sUID, string sPWD)
		{
			this.sLoginID = sUID;
			this.sPassword = sPWD;
			sPath = ConfigurationManager.AppSettings["LDAPpath"].ToString();  //moved to web.config rather than hard coding it
		...
		#region Check Group Level Membership
		public bool CheckGroupMembership(string[] aGroupsAllowedMembership)
		{
			string[] aUserGroupsMemberships = this.GetGroups().Split('|');
			bool bReturnMembership = false;

			for (int i=0;i<aUserGroupsMemberships.Length;i++)
			{
				for (int j=0;j<aGroupsAllowedMembership.Length;j++)
				{
					if (aGroupsAllowedMembership[j] == aUserGroupsMemberships[i])
					{
						bReturnMembership= true;
						break;
					}
				}

				if (bReturnMembership)
					break;
			}

			return bReturnMembership;
		}
		#endregion
