...
namespace DataEntryFramework4
{
	...
	public class DefaultFormEventHandler : IFormEventHandler
	{
		...
		*/ 
		protected SqlCommand GetUpdateSqlCommand(DataEntryController dec)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						_sqlUpdate += ((!first) ? "," : "") + d.FldName + "=@" + d.FldName;
						first = false;
					}
					...
					{
						_sqlUpdate += " exec def.spSCORE_after_save '" + dec.DatabaseTable + "', @" + dec.PrimaryKeyField + "; ";
					}
		...
		protected void AddParameters(SqlParameterCollection pcol, Dictionary<string, DataFieldControl> paramFields) 
		{
			//set parameters.  Parameter name is database fieldname with a prepended '@'
			foreach(DataFieldControl dfc in paramFields.Values) 
			{
		...
		protected void AddParameter(SqlParameterCollection pcol, DataFieldControl dfc) 
		{
			...
			pcol.AddWithValue("@" + dfc.FldName, val);
									...
									SqlCommand sqlCommand )
		{
			...
			{
				dfc.FieldTextBoxText = r[dfc.FldName].ToString();
			}
		...
		public virtual void DoSaveEnterering(DataEntryController dec)
		{
			...
			{
				dec.Notifications.Add(string.Format("Unexpected error in DoSaveEntering() event handler: {0}", ex.Message + "<br/><br/>Data: <br/>" + ex.Data + "<br/><br/>InnerException: <br/>" + ex.InnerException + "<br/><br/>SOURCE: <br/>" + ex.Source + "<br/>" + ex.StackTrace));
				dec.FormError = true;
			}
		}
		...
		public virtual void DoSaveSingleEntered(DataEntryController dec)
		{
			...
			try 
			{
				...
				{
					//dec.Notifications.Add(string.Format("dec.FormError = true<br/><br/>")); 
					dec.FormError = true;
				}
				else
				{
					//dec.Notifications.Add(string.Format("dec.FormError = false<br/><br/>"));
					dec.FormError = false;
					...
					//dec.Notifications.Add(string.Format("updateSqlCommand.CommandText = " + updateSqlCommand.CommandText + "<br/><br/>"));
			...
			catch (Exception ex)
			{
				...
				//dec.Notifications.Add(string.Format("Unexpected error in DoSaveSingleEntered() event handler: {0}", ex.Message + "<br/><br/>Target Site: <br/>" + ex.TargetSite + "<br/><br/>Data: <br/>" + ex.Data + "<br/><br/>InnerException: <br/>" + ex.InnerException + "<br/><br/>SOURCE: <br/>" + ex.Source + "<br/>" + ex.StackTrace));
				dec.FormError = true;
			}
		}
		...
		public virtual void DoVerifyCorrection(DataEntryController dec)
		{
			try 
			{
				...
				else
				{
					...
					foreach( DataFieldControl dfc in dec.DoubleEntryDataFields.Values) 
					{
			...
			if (errors == false)
			{
				...
				try 
				{
					...
					if (rowsAdded == 1)
					{
						...
						// save value of primary key
						string pkfld = dec.PrimaryKeyField;
						string pkval = r[pkfld].ToString();
						dec.PrimaryKeyVal = Convert.ToInt32(pkval) ;
