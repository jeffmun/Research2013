...
namespace DataEntryFramework4
{
	...
	public enum FormStates 
	{
		...
		DOUBLE_ENTERING_CORRECTING, DOUBLE_ENTERED, NO_DATA, EDITING_LAYOUT};

	
	[
	Designer(typeof(DataEntryFramework4.Design.DataEntryControllerDesigner)),
	...
	public class DataEntryController : WebControl, INamingContainer
	{
		...
		public List<string> PreValidateProperties()
		{
			...
			foreach (DataFieldControl dfc in AllDataFields.Values)
			{
				...
				{
					errors.Add("DataFieldControl for fldname='" + dfc.FldName + "' has an error: " + fErr);
				}
			}
		...
		public int StudyID
		{
			get
			{
				return ViewState["_StudyID"] == null ? -1 : (int)ViewState["_StudyID"];
			}

			set
			{
				ViewState["_StudyID"] = value;
			}
		}
				...
				if (LookupDataSet != null)
				{
					if (LookupDataSet.Tables["results"].Rows.Count > 1)
					{
						...
						foreach(DataRow r in LookupDataSet.Tables["results"].Rows)
						{
							...
							//add link
							t.Append("<td><a href='" + Page.Request.Url.GetLeftPart(UriPartial.Path) + "?studymeasID=" + this._studyMeasureDDL.SelectedValue.ToString() + "&pk=" + r[PrimaryKeyField].ToString() + "'>Open</a></td>");
		...
		private void SetupStudyMeasureDDL(  DropDownList ddl,   //lookup ddl
											...
											string midQSParam       //measureID querystring param  might be null or string.Empty
			...
			)
		{
			...
			try
			{
				...
				if (midQSParam != null && midQSParam != string.Empty)
				{
					sqlWhere += " and sm.measureID = " + midQSParam + " ";
				}
				...
				//If neither a mid or smid then do not return any
				if (string.IsNullOrEmpty(midQSParam) && string.IsNullOrEmpty(studymeasidQSParam))
				{
					sqlWhere = " where 1=2";
				}
				...
				//string sql = "select sm.studymeasid, s.studyname + ': ' + sm.studymeasname as studymeasure  " +
				//"from dbo.datTable t                                                           " +
				//"JOIN dbo.datTable_Measure tm ON t.tableID = tm.tableID                        " +
				//"join uwautism_research_backend..tblstudymeas sm on sm.measureID=tm.measureID  " +
				//"join uwautism_research_backend..tblstudy s on s.studyid=sm.studyid            " +
				//"left join datTable tAll on tAll.tableid=t.alltableid                          " + sqlWhere + sqlUnion;
		...
		private void SetupUI_ForEditing( )
		{


			//JM regardless of formstate, we want the linkButton visible 

			_studymeasLinkButton.Text = string.Format("Measures for {0}", this.StudySubjectIDString);
			...
			_blankmeasureButton.Visible = true;
			_blankmeasureButton.PostBackUrl = "~/dataentryforms/enterdata.aspx";
			...
			_stateLabel.Text = string.Format("**EDITING**: {0}", StudyMeasName);
							 ...
							 )
		{
			...
			switch (FormState)
			{
				case FormStates.EDITING_LAYOUT:
					_stateLabel.Text = string.Format("**EDITING**: {0}", MeasureName);
					_createNewButton.Visible = false;
					_insertButton.Visible = false;
					_saveButton.Visible = false;
					_doubleEnterButton.Visible = false;
		...
		[Category("Controller-Setup")]
		public string BlankMeasureButtonText
		{
			get
			{
				EnsureChildControls();
				return _blankmeasureButton.Text;
			}
			set
			{
				EnsureChildControls();
				_blankmeasureButton.Text = value;
			}
		}
			...
			if (ctl.HasControls())
			{
				foreach(Control c in ctl.Controls)
				{
					...
					if (c is DataFieldControl)
					{
						...
						//throw an error if dfc is missing fldname
						if (string.IsNullOrEmpty(dfc.FldName))
						{
							 //JMFIX throw new Exception("DataFieldControl with ID=" + dfc.ID + " is missing the required fldname property.");
						}
						else
						{
							// all 
							AllDataFields.Add(dfc.FldName.ToUpper(), dfc);

							// insert fields
							if (dfc.IsInsertField) { InsertDataFields.Add(dfc.FldName.ToUpper(), dfc); }

							// entry fields
							if (dfc.IsEntryField) { EntryDataFields.Add(dfc.FldName.ToUpper(), dfc); }

							// double entry fields
							if (dfc.IsDoubleEntryField) { DoubleEntryDataFields.Add(dfc.FldName.ToUpper(), dfc); }
		...
		protected override void OnLoad(EventArgs e)
		{
			...
			{
				string qsstudyid = Page.Request.QueryString["studyid"];
				if (string.IsNullOrEmpty(qsstudyid)) qsstudyid = this.StudyID.ToString();
		...
		protected override void OnPreRender(EventArgs e)
		{
			...
			{
				if (this.FormState== FormStates.EDITING_LAYOUT){
					SetupUI_ForEditing();
				}
			}
			base.OnPreRender (e);
