...
namespace DataEntryFramework3
{
	...
	public class DefaultFormEventHandler_preAug2013 : IFormEventHandler
	{
		...
		protected SqlCommand GetUpdateVerifiedSqlCommand(DataEntryController dec)
		{
			...
			else
			{
				...
				else
				{
                    ...
                    _sqlUpdateVerified += "SELECT t.*,  st.studyname + ': ' + sm.studymeasname as studymeasname  , uwautism_research_backend.dbo.fnGetSubjIDFromIDAndStudymeasID(ID, sm.studymeasID) as subjID  FROM " + dec.DatabaseTable + " t join uwautism_research_backend..tblstudymeas sm ";
                    ...
                    _sqlUpdateVerified += " WHERE  ID in (select ID from uwautism_research_backend.dbo.fnSEC_Allowed_TRACKING_IDs_by_User())   AND  " + dec.PrimaryKeyField + " = @" + dec.PrimaryKeyField;
