...
public partial class _test_DataProjectExplore : System.Web.UI.Page
{
	...
	protected void btnEditProject_OnClick(object sender, EventArgs e)
	{
		...
		Response.Redirect("DataProject.aspx?pk=" + Request.QueryString["pk"]);
	}
	...
	protected void btnGetStats_OnClick(object sender, EventArgs e)
	{
		...
		if (nrecs_proj_data > 0)
		{
			...
			if (chkOpt_desc.Checked)
			{
				...
				DataTable dt_descstats = ConvertSummaryDataSourceToDataTable(pivot_ds);
	...
	protected DataTable ConvertSummaryDataSourceToDataTable(PivotSummaryDataSource source)
	{
	...
	protected DataSet BuildDataSet()
	{
		...
		for (int t = 0; t < tbls.Count; t++)
		{
			...
			else if (Ns1 > 0 & Ns2 >= 0)
			{
				...
				string sqlcode_flds = String.Format("select fldname  from def.vwFld where fldpk in ({0}) and tblpk=(select tblpk from def.tbl where measureID={1} ) " +
				...
				else if (Ns2 > 0)
				{
					...
					msg = String.Format("X - Duplicate recs for {0} subject{1} excluded. [{2}]", Ns2, plural, dupsIDs_csv);
